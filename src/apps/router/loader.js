...
class Loader {
  ...
  dynamicWrapper(loadModel, models, component) {
    const app = this.app;
    // register models
    models.forEach(model => {
      if (modelNotExisted(app, model)) {
        // eslint-disable-next-line
        app.model(loadModel(model));
      }
    });
  ...
  load(routerConfig, loadModel) {
    ...
    Object.keys(routerConfig).forEach(path => {
      ...
      let router = {
        component: this.dynamicWrapper(loadModel, config.models, () => config.component),
      };
    ...
    routerDataCache = Object.assign({}, routerDataCache, routerData);
    return routerDataCache;
  }
}
